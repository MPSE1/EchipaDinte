...
public class Connect implements Runnable {
	...
	public static Image playerFrames[] = new Image[17];
	public static Image otherPlayerFrames[] = new Image[17];
	public static Image hydeFrames[] = new Image[17];
	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < 17; i++) {
				String fName = String.format("pacman_frames/pacman_%02d.png", i + 1);
				File file = new File(fName);
				playerFrames[i] = new Image(file.toURI().toString());
				if(playerFrames[i] == null)
				{
			...
			for (int i = 0; i < 17; i++) {
				String fName = String.format("player_frames/player_%02d.png", i + 1);
				File file = new File(fName);
				otherPlayerFrames[i] = new Image(file.toURI().toString());
				if(otherPlayerFrames[i] == null)
				{
					System.out.println("File " + fName + " not found!");
				}
			}
			...
			for (int i = 0; i < 17; i++) {
				String fName = String.format("hyde_frames/hyde_%02d.png", i + 1);
				File file = new File(fName);
				hydeFrames[i] = new Image(file.toURI().toString());
				if(hydeFrames[i] == null)
				{
					System.out.println("File " + fName + " not found!");
				}
			}
				...
				synchronized (Lock) {
					...
					for (int i = 0; i < numberOfPlayers; i++) {
						...
						if (i == playerNumber) {
							...
							if (state.doctor == 0) {
								ImagePattern ip = new ImagePattern(hydeFrames[frameIndex]);
								Main.playerRectangle.setFill(ip);
								Main.players.get(i).setFill(ip);
							} else {
								Thread.sleep(5);
								if(otherPlayerFrames[frameIndex] == null)
								{
									System.out.println("SPANAKE LALALALALA " + Integer.toString(frameIndex));
								}
								ImagePattern ipOther = new ImagePattern(otherPlayerFrames[frameIndex]);
								Main.players.get(i).setFill(ipOther);
								ImagePattern ip = new ImagePattern(playerFrames[frameIndex]);
								Main.playerRectangle.setFill(ip);
							}
						} else {
							...
							{
								ImagePattern ip = new ImagePattern(otherPlayerFrames[frameIndex]);
								Main.players.get(i).setFill(ip);
							}
						}
